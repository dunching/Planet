...
FString UCommonSession_HostSessionRequest::ConstructTravelURL() const
{
	...
	// return FString::Printf(TEXT("localhost:8888%s"),
...
void UCommonSessionSubsystem::HostSession(APlayerController* HostingPlayer, UCommonSession_HostSessionRequest* Request)
{
	...
	CurrentSessionName = Request->SessionName;
	...
	{
		CreateOnlineSessionInternal(HostingPlayer, LocalPlayer, Request);
	}
...
void UCommonSessionSubsystem::CreateOnlineSessionInternal(
	APlayerController* HostingPlayer, ULocalPlayer* LocalPlayer, UCommonSession_HostSessionRequest* Request
	)
{
	CreateSessionResult = FOnlineResultInformation();
	PendingTravelURL = Request->ConstructTravelURL();

	RequestCreateSessionPCPtr = HostingPlayer;
...
void UCommonSessionSubsystem::CreateOnlineSessionInternalOSSv1(ULocalPlayer* LocalPlayer, UCommonSession_HostSessionRequest* Request)
{
	...
	const FName NewSessionName(CurrentSessionName);
	...
	if (ensure(UserId.IsValid()))
	{
		...
		HostSettings->Set(SETTING_SESSION_TEMPLATE_NAME, CurrentSessionName, EOnlineDataAdvertisementType::DontAdvertise);
		HostSettings->Set(SETTING_ONLINESUBSYSTEM_VERSION, true, EOnlineDataAdvertisementType::ViaOnlineService);
		HostSettings->Set(SETTING_BEACONPORT, 8888, EOnlineDataAdvertisementType::ViaOnlineService);
		
		HostSettings->Set(TEXT("SessionName"), CurrentSessionName, EOnlineDataAdvertisementType::ViaOnlineService);

		Sessions->CreateSession(*UserId, NewSessionName, *HostSettings);
		NotifySessionInformationUpdated(ECommonSessionInformationState::InGame, Request->ModeNameForAdvertisement, Request->GetMapName());
	}
	else
	{
		OnCreateSessionComplete(NewSessionName, false);
	}
}
...
void UCommonSessionSubsystem::HandleQuickPlaySearchFinished(bool bSucceeded, const FText& ErrorMessage, TWeakObjectPtr<APlayerController> JoiningOrHostingPlayer, TStrongObjectPtr<UCommonSession_HostSessionRequest> HostRequest)
{
	...
	if (bSucceeded || ErrorMessage.IsEmpty())
	{
		...
		if (ResultCount > 0)
		{
			...
			{
				JoinSession(
					JoiningOrHostingPlayer.Get(), HostRequest->SessionName,CurrentSessionName, Result
					);
				return;
			}
		}
...
void UCommonSessionSubsystem::JoinSession(
		APlayerController* JoiningPlayer, 
		const FString& InPendingSessionName,
		...
		)
{
	...
	PendingSessionName = InPendingSessionName;
...
void UCommonSessionSubsystem::JoinSessionInternalOSSv1(ULocalPlayer* LocalPlayer, UCommonSession_SearchResult* Request)
{
	...
	Sessions->JoinSession(*LocalPlayer->GetPreferredUniqueNetId().GetUniqueNetId(), *PendingSessionName, Request->Result);
}
...
void UCommonSessionSubsystem::FinishJoinSession(EOnJoinSessionCompleteResult::Type Result)
{
	if (Result == EOnJoinSessionCompleteResult::Success)
	{
		...
		InternalTravelToSession(*PendingSessionName/*NAME_GameSession*/);
	}
	else
	{
		FText ReturnReason;
...
void UCommonSessionSubsystem::InternalTravelToSession(const FName SessionName)
{
	...
	if (!Sessions->GetResolvedConnectString(SessionName, URL, NAME_BeaconPort))
	{
	...
	if (auto OnlineSessionSettingsPtr = Sessions->GetSessionSettings(SessionName))
	{
		FString MapName;
		if (OnlineSessionSettingsPtr->Get(SETTING_MAPNAME, MapName))
		{
			// URL = FString::Printf(TEXT("%s?bIsLanMatch?listen"), *MapName);
			// URL = MapName;
		}
		else
		{
