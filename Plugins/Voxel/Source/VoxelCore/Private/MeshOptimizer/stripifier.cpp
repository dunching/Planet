...
{
	checkVoxelSlow(destination != indices);
	...
	{
		checkVoxelSlow(next < 0 || (size_t(next >> 2) < buffer_size && (next & 3) < 3));
...
{
	checkVoxelSlow(index_count == 0 || index_count >= 3);
