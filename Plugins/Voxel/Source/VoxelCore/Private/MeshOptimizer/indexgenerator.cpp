...
size_t meshopt_generateVertexRemap(unsigned int* destination, const unsigned int* indices, size_t index_count, const void* vertices, size_t vertex_count, size_t vertex_size)
{
	...
	checkVoxelSlow(indices || index_count == vertex_count);
	...
	for (size_t i = 0; i < index_count; ++i)
	{
		...
		if (destination[index] == ~0u)
		{
			...
			{
				checkVoxelSlow(destination[*entry] != ~0u);
	...
	checkVoxelSlow(next_vertex <= vertex_count);
...
size_t meshopt_generateVertexRemapMulti(unsigned int* destination, const unsigned int* indices, size_t index_count, size_t vertex_count, const struct meshopt_Stream* streams, size_t stream_count)
{
	...
	checkVoxelSlow(indices || index_count == vertex_count);
	checkVoxelSlow(index_count % 3 == 0);
	checkVoxelSlow(stream_count > 0 && stream_count <= 16);

	for (size_t i = 0; i < stream_count; ++i)
	{
		checkVoxelSlow(streams[i].size > 0 && streams[i].size <= 256);
		checkVoxelSlow(streams[i].size <= streams[i].stride);
	}
...
void meshopt_remapVertexBuffer(void* destination, const void* vertices, size_t vertex_count, size_t vertex_size, const unsigned int* remap)
{
	...
	for (size_t i = 0; i < vertex_count; ++i)
	{
		if (remap[i] != ~0u)
		{
			checkVoxelSlow(remap[i] < vertex_count);
...
void meshopt_generateShadowIndexBuffer(unsigned int* destination, const unsigned int* indices, size_t index_count, const void* vertices, size_t vertex_count, size_t vertex_size, size_t vertex_stride)
{
	...
	checkVoxelSlow(indices);
	checkVoxelSlow(index_count % 3 == 0);
	checkVoxelSlow(vertex_size > 0 && vertex_size <= 256);
	checkVoxelSlow(vertex_size <= vertex_stride);
...
void meshopt_generateAdjacencyIndexBuffer(unsigned int* destination, const unsigned int* indices, size_t index_count, const float* vertex_positions, size_t vertex_count, size_t vertex_positions_stride)
{
	...
	for (size_t i = 0; i < index_count; i += 3)
	{
		for (int e = 0; e < 3; ++e)
		{
			...
			checkVoxelSlow(i0 < vertex_count && i1 < vertex_count && i2 < vertex_count);
	...
	for (size_t i = 0; i < index_count; i += 3)
	{
		...
		for (int e = 0; e < 3; ++e)
		{
			unsigned int i0 = indices[i + e];
			unsigned int i1 = indices[i + next[e]];
			checkVoxelSlow(i0 < vertex_count && i1 < vertex_count);
