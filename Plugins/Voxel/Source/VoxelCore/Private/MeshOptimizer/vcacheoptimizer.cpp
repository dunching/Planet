...
{
	checkVoxelSlow(cache_position >= -1 && cache_position < int(kCacheSizeMax));
...
void meshopt_optimizeVertexCacheTable(unsigned int* destination, const unsigned int* indices, size_t index_count, size_t vertex_count, const meshopt::VertexScoreTable* table)
{
	...
	checkVoxelSlow(cache_size <= kCacheSizeMax);
	...
	{
		checkVoxelSlow(output_triangle < face_count);
		...
		for (size_t i = 0; i < cache_write; ++i)
		{
			...
			{
				unsigned int tri = *it;
				checkVoxelSlow(!emitted_flags[tri]);

				float tri_score = triangle_scores[tri] + score_diff;
				checkVoxelSlow(tri_score > 0);
	...
	checkVoxelSlow(input_cursor == face_count);
	checkVoxelSlow(output_triangle == face_count);
}
