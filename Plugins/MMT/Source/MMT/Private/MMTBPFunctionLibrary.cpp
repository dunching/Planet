...
USceneComponent* UMMTBPFunctionLibrary::GetSceneComponentReferenceByName(UActorComponent* Target, FString Name)
{
	if (IsValid(Target))
	{
		AActor* Owner = Target->GetOwner();
		TArray<USceneComponent*> FoundComponents;
		Owner->GetComponents<USceneComponent>(FoundComponents);
		...
		for (int32 i = 0; i < FoundComponents.Num(); i++)
		{
			...
			{
				Result = Cast<USceneComponent>(FoundComponents[i]);
				if (IsValid(Result))
				{
					return Result;
				}
...
UMeshComponent* UMMTBPFunctionLibrary::GetMeshComponentReferenceByName(UActorComponent* Target, FString Name)
{
	if (IsValid(Target))
	{
		...
		Owner->GetComponents<UMeshComponent>(FoundComponents);
...
USplineComponent* UMMTBPFunctionLibrary::GetSplineComponentReferenceByName(UActorComponent* Target, FString Name)
{
	if (IsValid(Target))
	{
		...
		Owner->GetComponents<USplineComponent>(FoundComponents);
