	...
	)
{
	const auto CharacterProxySPtr =
		GetOwner<FOwnerType>()->GetInventoryComponent()->FindProxy<FModifyItemProxyStrategy_Character>(ProxtID);
...
void UTeamMatesHelperComponent::SpwanTeammateCharacter_Server_Implementation()
{
	...
	for (int32 Index = 0; Index < CharactersAry.Num(); Index++)
	{
		...
		{
			const auto CharacterProxySPtr =
				GetOwner<FOwnerType>()->GetInventoryComponent()->FindProxy<FModifyItemProxyStrategy_Character>(
