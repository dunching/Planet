...
UPlanetChildActorComponent::UPlanetChildActorComponent(const FObjectInitializer& ObjectInitializer) :
...
void UPlanetChildActorComponent::CreateChildActor(TFunction<void(AActor*)> CustomizerFunc /*= nullptr*/)
{
	...
	if ((World->IsGameWorld()))
	{
		...
		if (GetNetMode() == NM_DedicatedServer)
		{
			...
			if (!OnwerActorPtr->bDirctSpawnChild)
			{
				return;
			}
			...
			Params.CustomPreSpawnInitalization = [OnwerActorPtr](AActor* ActorPtr)
			{
				PRINTINVOKEINFO();
