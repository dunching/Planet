...
) const
{
	...
	{
		InstanceData.GloabVariable->SpwanedCharacterAry.Empty();
		...
		for (auto Iter : Components)
		{
			...
			))
			{
				...
				if (ChildActorPtr)
				{
					InstanceData.GloabVariable->SpwanedCharacterAry.Add(
						ChildActorPtr
					);

					InstanceData.GloabVariable->TemporaryActorAry.Add(
						ChildActorPtr
					);
				}
				else
...
) const
{
	...
	if (InstanceData.GloabVariable->SpwanedCharacterAry.IsEmpty())
	{
		return EStateTreeRunStatus::Failed;
	}
...
) const
{
	...
	{
		for (auto Iter : InstanceData.GloabVariable->SpwanedCharacterAry)
		{
			if (Iter.IsValid())
			{
