...
namespace TestCommand
{
	...
	PLANET_API void HasBeenRepel(const TArray< FString >& Args);

	static FAutoConsoleCommand HasBeenRepelCMD(
		//CMD 名字，在控制台输入这个调用
		TEXT("HasBeenRepel"),
		//控制台帮助信息，选择这个命令的时候会看到
		TEXT("this is a CMD test."),
		//创建静态委托，输入上面的命令后会调用到后面的函数
		FConsoleCommandWithArgsDelegate::CreateStatic(HasBeenRepel),
		//可选标志位掩码
		EConsoleVariableFlags::ECVF_Default
	);
	
	void StunTarget(const TArray< FString >& Args);

	static FAutoConsoleCommand StunTargetCMD(
		//CMD 名字，在控制台输入这个调用
		TEXT("StunTarget"),
		//控制台帮助信息，选择这个命令的时候会看到
		TEXT("this is a CMD test."),
		//创建静态委托，输入上面的命令后会调用到后面的函数
		FConsoleCommandWithArgsDelegate::CreateStatic(StunTarget),
		//可选标志位掩码
		EConsoleVariableFlags::ECVF_Default
	);
	...
	static FAutoConsoleCommand SetCharacterAttributeValueCMD(
	 ...
	 void SetCharacterAttributeTemporaryValue(const TArray< FString >& Args);

	static FAutoConsoleCommand SetCharacterAttributeTemporaryValueCMD(
		//CMD 名字，在控制台输入这个调用
		TEXT("SetCharacterAttributeTemporaryValue"),
		//控制台帮助信息，选择这个命令的时候会看到
		TEXT("this is a CMD test."),
		//创建静态委托，输入上面的命令后会调用到后面的函数
		FConsoleCommandWithArgsDelegate::CreateStatic(SetCharacterAttributeTemporaryValue),
		//可选标志位掩码
		EConsoleVariableFlags::ECVF_Default
	);
	
	PLANET_API void UpdateWeather(const TArray< FString >& Args);

	static FAutoConsoleCommand UpdateWeatherCMD(
		//CMD 名字，在控制台输入这个调用
		TEXT("UpdateWeather"),
		//控制台帮助信息，选择这个命令的时候会看到
		TEXT("this is a CMD test."),
		//创建静态委托，输入上面的命令后会调用到后面的函数
		FConsoleCommandWithArgsDelegate::CreateStatic(UpdateWeather),
		//可选标志位掩码
		EConsoleVariableFlags::ECVF_Default
	);
};