...
void UAbilityTask_ASCPlayMontage::OnMontageBlendingOut(
	UAnimMontage* Montage,
	bool bInterrupted
	)
{
	const bool bPlayingThisMontage = (Montage == MontageToPlay) && Ability && Ability->GetCurrentMontage() ==
	                                 MontageToPlay;
...
void UAbilityTask_ASCPlayMontage::OnMontageEnded(
	UAnimMontage* Montage,
	bool bInterrupted
	)
{
...
void UAbilityTask_ASCPlayMontage::PlayMontage(
	...
	)
{
	...
	if (UAbilitySystemComponent* ASC = AbilitySystemComponent.Get())
	{
		...
		if (AnimInstance != nullptr)
		{
			                    ...
			                    ))
			{
				...
				if (FAnimMontageInstance* MontageInstance = AnimInstance->GetActiveInstanceForMontage(InMontageToPlay))
				{
					MontageInstanceID = MontageInstance->GetInstanceID();
				}

				BlendingOutDelegate.BindUObject(this, &ThisClass::OnMontageBlendingOut);
				AnimInstance->Montage_SetBlendingOutDelegate(BlendingOutDelegate, InMontageToPlay);

				MontageEndedDelegate.BindUObject(this, &ThisClass::OnMontageEnded);
				AnimInstance->Montage_SetEndDelegate(MontageEndedDelegate, InMontageToPlay);
	...
	if (!bPlayedMontage)
	{
		ABILITY_LOG(
		            Warning,
		            TEXT(
			            "UAbilityTask_PlayMontage called in Ability %s failed to play montage %s; Task Instance Name %s."
		            ),
		            *Ability->GetName(),
...
{
	PlayMontage(MontageToPlay, StartSection);
}
...
bool UAbilityTask_ASCPlayMontage::IsNotifyValid(
	FName NotifyName,
...
void UAbilityTask_ASCPlayMontage::OnNotifyBeginReceived(
	FName NotifyName,
	const FBranchingPointNotifyPayload& BranchingPointNotifyPayload
	)
{
...
void UAbilityTask_ASCPlayMontage::OnNotifyEndReceived(
	FName NotifyName,
	const FBranchingPointNotifyPayload& BranchingPointNotifyPayload
	)
{
...
void UAbilityTask_ASCPlayMontage::OnDestroy(
	bool AbilityEnded
	)
{
