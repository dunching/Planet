...
APlanetPlayerController::APlanetPlayerController(
	const FObjectInitializer& ObjectInitializer
	  ...
	  Super(
	        ObjectInitializer.
	        SetDefaultSubobjectClass<AGroupManagger_Player>(
	                                                        AGroupManagger_Player::ComponentName
	                                                       )
	       )
{
	...
	for (auto Iter : OutActors)
	{
		...
		if (CharacterPtr)
		{
			CharacterPtr->ForEachComponent(
			                               ...
			                               {
				                               auto GroupSharedInterfacePtr = Cast<IGroupManaggerInterface>(
					                                ComponentPtr
					                               );
				                               if (GroupSharedInterfacePtr)
				                               {
					                               GroupSharedInterfacePtr->
						                               OnPlayerGroupManaggerReady(GroupManaggerPtr);
				                               }
			                               }
			                              );
		}
...
AGroupManagger_Player* APlanetPlayerController::GetGroupManagger_Player() const
{
	return Cast<AGroupManagger_Player>(GroupManaggerPtr);
}
