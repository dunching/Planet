...
void UStateSwitchComponent::SwitchToNewState(EBuildingState BuildingState_)
{
	...
	case EBuildingState::kPreviewPlacing_Sucess:
	{
		const auto MaterialPtr =
			UAssetRefMap::GetInstance()->MaterialInsSoftPath[EMaterialInstanceType::kPlacePreviewMatIns_Success].LoadSynchronous();
	...
	{
		const auto MaterialPtr =
			UAssetRefMap::GetInstance()->MaterialInsSoftPath[EMaterialInstanceType::kPlacePreviewMatIns_Fail].LoadSynchronous();
...
FItemType UBuildingPropertyComponent::GetItemsType() const
{
	return ItemType;
}
...
UBuildingPropertyComponent::UBuildingPropertyComponent(const FObjectInitializer& ObjectInitializer) :
	Super(ObjectInitializer)
{
	ItemType.Enumtype = EEnumtype::kBuilding;
	ItemType.ItemType = BuildingType;
}
