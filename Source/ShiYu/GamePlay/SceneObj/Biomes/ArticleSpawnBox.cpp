...
void AArticleSpawnBox::BeginPlay()
{
    ...
    auto GameModePtr = Cast<AShiYuGameMode>(UGameplayStatics::GetGameMode(this));
    if (!GameModePtr)
    {
        return;
    }
...
{
	auto GameModePtr = Cast<AShiYuGameMode>(UGameplayStatics::GetGameMode(this));
	if (GameModePtr && bIsReSpawn)
	{
		TArray<AActor*>ResultAry;

		UGameplayStatics::GetAllActorsOfClass(GameModePtr, ARawMaterialBase::StaticClass(), ResultAry);
