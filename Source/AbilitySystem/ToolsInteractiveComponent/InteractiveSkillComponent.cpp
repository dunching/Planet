...
void UInteractiveSkillComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	auto OwnerCharacterInterfacePtr = GetOwner<IGAOwnerInterface>();
	auto OwnerCharacterPtr = GetOwner<FOwnerType>();
	if (OwnerCharacterPtr)
	{
		auto GASPtr = OwnerCharacterInterfacePtr->GetAbilitySystemComponent();
		...
		if (WeaponUnit)
		{
			auto GASpecPtr = OwnerCharacterInterfacePtr->GetAbilitySystemComponent()->FindAbilitySpecFromHandle(WeaponUnit->Handle);
			if (GASpecPtr)
			{
...
void UInteractiveSkillComponent::InitialBaseGAs()
{
	...
	if (OwnerCharacterPtr)
	{
		...
		GASPtr->InitAbilityActorInfo(OwnerCharacterPtr, OwnerCharacterPtr);
	...
	if (InWeaponSocket != CurrentActivedWeaponSocket)
	{
		...
		// 清除上一次的
		{
			...
			if (WeaponUnit)
			{
				...
				OwnerCharacterInterfacePtr->GetAbilitySystemComponent()->ClearAbility(WeaponUnit->Handle);
			}
			if (ActivedWeaponPtr)
			{
		...
		CurrentActivedWeaponSocket = InWeaponSocket;
		{
			...
			{
				WeaponUnit->Handle = OwnerCharacterInterfacePtr->GetAbilitySystemComponent()->GiveAbility(
					FGameplayAbilitySpec(
				...
				ActorSpawnParameters.Owner = OwnerCharacterPtr;
...
void UInteractiveSkillComponent::CancelSkill_WeaponActive(const TSharedPtr<FCanbeActivedInfo>& CanbeActivedInfoSPtr)
{
	...
	auto ASCPtr = OwnerCharacterInterfacePtr->GetAbilitySystemComponent();
	...
	for (const auto& Iter : InSkillsMap)
	{
		...
		if (PreviouIter && *PreviouIter && (*PreviouIter)->SkillUnit)
		{
			...
			else
			{
				...
				case ESkillType::kPassive:
				{
					...
					if (PassiveSkillUnitPtr)
					{
						for (const auto& ElementIter : PassiveSkillUnitPtr->AddtionalElementMap)
						{
							...
							{
								OwnerCharacterInterfacePtr->GetCharacterAttributes().Element.GoldElement.RemoveCurrentValue(
				...
				{
					OwnerCharacterInterfacePtr->GetCharacterAttributes().TalentSPtr.Reset();
				}
				default:
				{
					for (const auto SkillHandleIter : Iter.Value->HandleAry)
					{
						OwnerCharacterInterfacePtr->GetAbilitySystemComponent()->ClearAbility(SkillHandleIter);
					}
		...
		if (Iter.Value && Iter.Value->SkillUnit)
		{
			...
			case ESkillType::kPassive:
			{
				...
				if (PassiveSkillUnitPtr)
				{
					if (PassiveSkillUnitPtr->AddtionalElementMap.IsEmpty())
					{
						...
						Ref->HandleAry.Add(OwnerCharacterInterfacePtr->GetAbilitySystemComponent()->GiveAbility(
							FGameplayAbilitySpec(
					...
					else
					{
						for (const auto& ElementIter : PassiveSkillUnitPtr->AddtionalElementMap)
						{
							...
							case EWuXingType::kGold:
							{
								OwnerCharacterInterfacePtr->GetCharacterAttributes().Element.GoldElement.AddCurrentValue(
									ElementIter.Value,
									PassiveSkillUnitPtr->PropertuModify_GUID
								);
							}
			...
			case ESkillType::kTalentPassive:
			{
				...
				case ESkillUnitType::kHumanSkill_Talent_NuQi:
				{
					OwnerCharacterInterfacePtr->GetCharacterAttributes().TalentSPtr = MakeShared<FTalent_NuQi>();
				}
				break;
				case ESkillUnitType::kHumanSkill_Talent_YinYang:
				{
					OwnerCharacterInterfacePtr->GetCharacterAttributes().TalentSPtr = MakeShared<FTalent_YinYang>();
				}
				break;
				}
				switch (Iter.Value->SkillUnit->GetSceneElementType<ESkillUnitType>())
