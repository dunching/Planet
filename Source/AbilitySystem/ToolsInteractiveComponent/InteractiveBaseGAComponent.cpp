...
void UInteractiveBaseGAComponent::InitialBaseGAs()
{
	...
	auto OwnerCharacterInterfacePtr = GetOwner<FOwnerInterfaceType>();
	if (OwnerCharacterPtr && OwnerCharacterInterfacePtr)
	{
			...
			for (auto& Iter : GameplayAbilityTargetData_GAEvent.DataAry)
			{
				if (Iter.ElementSet.IsEmpty())
				{
					const auto& CharacterAttributes =
						Cast<FOwnerInterfaceType>(GameplayAbilityTargetData_GAEvent.TriggerCharacterPtr)->GetCharacterAttributes();
...
void UInteractiveBaseGAComponent::AddReceivedWuXingModify()
{
	struct GAEventModify_MultyTarget : public IGAEventModifyReceivedInterface
	{
		...
		virtual void Modify(FGameplayAbilityTargetData_GAReceivedEvent& GameplayAbilityTargetData_GAEvent)override
		{
			...
			if (DataRef.ElementSet.IsEmpty() && DataRef.TargetCharacterPtr)
			{
				const auto& CharacterAttributes =
					Cast<FOwnerInterfaceType>(DataRef.TargetCharacterPtr)->GetCharacterAttributes();
...
void UInteractiveBaseGAComponent::AddReceivedModify()
{
	struct GAEventModify_MultyTarget : public IGAEventModifyReceivedInterface
	{
		...
		virtual void Modify(FGameplayAbilityTargetData_GAReceivedEvent& GameplayAbilityTargetData_GAEvent)override
		{
			if (GameplayAbilityTargetData_GAEvent.Data.GetTargetCharacter<FOwnerInterfaceType>())
			{
				const auto& SelfCharacterAttributes =
					GameplayAbilityTargetData_GAEvent.Data.GetTargetCharacter<FOwnerInterfaceType>()->GetCharacterAttributes();

				const auto& TargetCharacterAttributes =
					GameplayAbilityTargetData_GAEvent.GetTriggerCharacter<FOwnerInterfaceType>()->GetCharacterAttributes();
