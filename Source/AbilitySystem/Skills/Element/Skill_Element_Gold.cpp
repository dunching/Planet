...
)
{
	...
	if (CharacterPtr)
	{
		auto CharacterAttributes = CharacterInterfacePtr->GetCharacterAttributes();
		...
		AbilityActivatedCallbacksHandle =
			CharacterInterfacePtr->GetAbilitySystemComponent()->AbilityActivatedCallbacks.AddUObject(this, &ThisClass::OnSendAttack);
	}
}
...
{
	CharacterInterfacePtr->GetAbilitySystemComponent()->AbilityActivatedCallbacks.Remove(AbilityActivatedCallbacksHandle);
...
void USkill_Element_Gold::OnSendAttack(UGameplayAbility* GAPtr)
{
	if (CharacterPtr)
	{
		if (!(
			GAPtr &&
			(GAPtr->GetCurrentAbilitySpecHandle() == CharacterInterfacePtr->GetInteractiveBaseGAComponent()->SendEventHandle)
			))
		{
			return;
		}
